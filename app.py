...
# ── Chunking config ────────────────────────────────────────────────────────
BRD_CHUNK_SIZE   = 4_000   # larger — BRD just needs reading/summarising
EPIC_CHUNK_SIZE  = 2_000   # smaller — Epic List is the estimation source
CHUNK_OVERLAP    = 100

# Log the active AI config at startup
_cfg = ai_client.get_config()
logger.info("AI client: base_url=%s  model=%s", _cfg["base_url"], _cfg["model"])
...
def extract_text(tmp_path: str, ext: str, mime: str) -> str:
    """Load a document with the appropriate LangChain loader and return plain text."""
...
def run_estimation(
    ...
    # ── Phase 1: feed intermediate Epic List chunks with no history ────────
    ...
    # ── Phase 2: final Epic chunk → basic estimate ────────────────────────
    ...
    logger.info("Final epic chunk sent | response: %d chars", len(basic_text))
    ...
    # ── Phase 3: contextual follow-up ──────────────────────────────────
    logger.info("Sending contextual follow-up…")
    history = [
...
def estimate():
    ...
    try:
        # ── 3. Extract Epic List text (required) ─────────────────────────
        ...
        epic_chunks = chunk_text(epic_list_text)
        if brd_chunks:
